...
public enum TriangleReveal {
  ...
  public static func sample(elapsed: TimeInterval, reduceMotion: Bool) -> Frame {
    ...
    let bob: Float = reduceMotion ? 0 : 0.006 * sin(time * 1.7) * progress
    let initial =
    ...
    let roll: Float = reduceMotion ? 0 : 0.015 * sin(time * 1.3) * progress
    return Frame(
      position: [0, -0.10 * (1 - movement) + bob, z],
      orientation: simd_quatf(angle: roll, axis: [0, 0, 1]) * rotation,
